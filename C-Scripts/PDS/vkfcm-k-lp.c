...
VKFCM-K-LP -  Partial Distance Strategy (PDS)
Version: 0.1
...
int noMissing(struct dados *d, int k);
...
void inicial_prototype(struct partition *p, struct dados *d, int *sel)
{
    ...
    {
        for(j=0; j<(d->n_var-1);j++){
			if(d->x[sel[i]][j+1]!= 9999){
            p->p[i].protot[j] = d->x[sel[i]][j+1]; // selecionando os prot. de acordo com os sorteados
		}else{
			 p->p[i].protot[j] = d->x[sel[i]+1][j+1]; //pegando a próxima linha caso tenha algum missing
    ...
    for(i=0; i<p->n_clust;i++)
    {
        for(j=0; j<(d->n_var-1);j++)
        {
            ...
            for(k=0;k<d->n_ind;k++)
            {
				if(d->x[k][j+1]!=9999){
				I=1;
                num = num + (pow(p->pert[k][i],m) * exp(-0.5* (d->x[k][j+1] - p->p[i].protot[j]) * (d->x[k][j+1] - p->p[i].protot[j])/sigma[j]) * d->x[k][j+1])*I;
                den = den + (pow(p->pert[k][i],m) * exp(-0.5* (d->x[k][j+1] - p->p[i].protot[j]) * (d->x[k][j+1] - p->p[i].protot[j])/sigma[j]))*I;
               }else{
			     I=0;
                num = num + (pow(p->pert[k][i],m) * exp(-0.5* (d->x[k][j+1] - p->p[i].protot[j]) * (d->x[k][j+1] - p->p[i].protot[j])/sigma[j]) * d->x[k][j+1])*I;
                den = den + (pow(p->pert[k][i],m) * exp(-0.5* (d->x[k][j+1] - p->p[i].protot[j]) * (d->x[k][j+1] - p->p[i].protot[j])/sigma[j]))*I;
        ...
        for(i=0;i<d->n_ind;i++)
        {
            ...
            for(j=0;j<(d->n_var-1);j++)
            {
				if(d->x[i][j+1]!= 9999){
					 I=1;
                    cont=noMissing(d,i);
                s = s + (p->p[k].weight[j] * 2.0 * (1.0 - exp(-0.5* ((d->n_var-1)/cont)*(d->x[i][j+1] - p->p[k].protot[j]) * (d->x[i][j+1] - p->p[k].protot[j])/sigma[j])))*I;
			}else{
				I=0;
				s = s + (p->p[k].weight[j] * 2.0 * (1.0 - exp(-0.5* ((d->n_var-1)/cont)*(d->x[i][j+1] - p->p[k].protot[j]) * (d->x[i][j+1] - p->p[k].protot[j])/sigma[j])))*I;
